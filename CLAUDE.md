...
**Native dependencies**: `libwhisper/whisper.cpp` (git submodule, CMake), `asian-autocorrect` (git submodule, Rust → `libautocorrect_swift.dylib` via `AutocorrectWrapper`), sherpa-onnx + onnxruntime (fetched/vendored under `vendor/`), FluidAudio (Swift package, patched at build time).
