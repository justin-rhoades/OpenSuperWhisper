...
**Voice commands** (optional): in the hotkey path (`IndicatorWindow`), a transcription that starts with the trigger word ("whisper open slack") is routed by `VoiceCommandRouter` to an action (open/focus/quit an app via `NSWorkspace`, resolved through `InstalledApps.bestMatch`) **instead of being typed** — it short-circuits before LLM cleanup/insertion. The router is also the home for the trailing "press enter" submit parse. Opt-in via `voiceCommandsEnabled`; trigger word configurable.
